	...
	t.prototype = {
		...
		getElementPos: function(a) {
			...
			var c = navigator.userAgent.toLowerCase(),
				b = null,
				d = [];
			if (a.getBoundingClientRect) return c = a.getBoundingClientRect(), a = Math.max(document.documentElement.scrollTop, document.body.scrollTop), b = Math.max(document.documentElement.scrollLeft, document.body.scrollLeft), {
			...
			if (document.getBoxObjectFor) c = document.getBoxObjectFor(a), d = [c.x - (a.style.borderLeftWidth ? Math.floor(a.style.borderLeftWidth) : 0), c.y - (a.style.borderTopWidth ? Math.floor(a.style.borderTopWidth) : 0)];
				...
				if (-1 < c.indexOf("opera") || -1 < c.indexOf("safari") && "absolute" == a.style.position) d[0] -= document.body.offsetLeft, d[1] -= document.body.offsetTop
			}
